...
import { AddressType, GovernorateItemType, ResultFlowDataType } from "types"
...
const PersonalInfo: React.FC<Props> = ({
  ...
  const handleSubmitModal = (item: Partial<AddressType>) => {
    if (item.city) {
    ...
    if (item.governorate) {
      setValue("governorate", item.governorate, {
        shouldValidate: true,
        shouldDirty: true,
        shouldTouch: true,
      })
